...
def get_evaluate_accuracy_reasoning_memorizing(
  ...
  process_hidden_states_mode = str(ProcessHiddenStatesMode.FIRST_ANSWER_TOKEN) if with_intervention else ""
  ...
  direction_normalization_mode = str(DirectionNormalizationMode.UNIT_VECTOR) if with_intervention else ""
  direction_normalization_mode_arg = f"--direction_normalization_mode {direction_normalization_mode}" if with_intervention else ""
  
  projection_hook_mode = str(ProjectionHookMode.FEATURE_ADDITION) if with_intervention else ""
