...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Main recurrence
        x, num_steps_no_grad, num_steps_with_grad, xk, block_idx, all_hidden_states = self.iterate_forward(
    ...
    def iterate_forward(
        ...
        with torch.no_grad():
            ...
            for no_grad_step in range(num_steps_no_grad):
                xk = x
                x, block_idx, all_hidden_states = self.core_block_forward(
                    xk, input_embeds, freqs_cis, mask, past_key_values, block_idx, no_grad_step,
                    return_head, all_hidden_states,
                )

        for grad_step in range(num_steps_with_grad):
            xk = x
            x, block_idx, all_hidden_states = self._maybe_checkpoint_core_block(
                xk, input_embeds, freqs_cis, mask, past_key_values, block_idx, num_steps_no_grad + grad_step,
                return_head, all_hidden_states,
            )
        return self.transformer.ln_f(x), num_steps_no_grad, num_steps_with_grad, xk.detach(), block_idx, all_hidden_states  # type: ignore # types broken in 2.6+
    ...
    def core_block_forward(
        ...
        return x, block_idx, all_hidden_states
    ...
    def iterate_one_step(
        ...
        if position_ids is None and cache_position is None:
            ...
            freqs_cis = self.freqs_cis[:, cache_position]
        x, block_idx, _ = self.core_block_forward(
            input_states,
            input_embeds,
            freqs_cis,
            attention_mask,
